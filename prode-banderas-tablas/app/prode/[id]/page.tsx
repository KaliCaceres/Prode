        ...
        <tbody>
          {posiciones.map((p, i) => (
            <tr key={p.equipo} style={{ background: i < 2 ? '#f0faf4' : undefined, borderTop: '1px solid #f0efec' }}>
              ...
              <td style={{ padding: '4px 6px', fontWeight: 500 }}>{bandera(p.equipo)} {p.equipo}</td>
        ...
        {GRUPOS.map(grupo => (
          <div key={grupo.letra} style={{ background: '#fff', border: '1px solid var(--gris-borde)', borderRadius: 'var(--radio)', overflow: 'hidden' }}>
            <div style={{ background: 'var(--gris-header)', color: '#fff', padding: '10px 16px', display: 'flex', alignItems: 'baseline', gap: '10px' }}>
              ...
              <span style={{ fontSize: '11px', opacity: 0.6 }}>{grupo.equipos.map(e => bandera(e)).join(' ')}</span>
            </div>
            {grupo.partidos.map(partido => {
              ...
              return (
                <div key={partido.id} style={{
                  ...
                  <span style={{ fontSize: '12px', textAlign: 'right' }}>{bandera(partido.local)} {partido.local}</span>
                  {editando ? (
                  ...
                  <span style={{ fontSize: '12px' }}>{partido.visitante} {bandera(partido.visitante)}</span>
